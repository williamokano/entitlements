...
import { ColumnDef, createColumnHelper, getCoreRowModel, getFilteredRowModel, getPaginationRowModel, getSortedRowModel, SortingState, Row as TableRow, Table as TableType, useReactTable } from '@tanstack/react-table'
...
const SellerProducts = () => {
  const columns: ColumnDef<SellerProductType, any>[] = [
    {
      ...
      maxSize: 45,
      size: 45,
      header: ({ table }: { table: TableType<SellerProductType> }) => <input type="checkbox" className="form-checkbox form-checkbox-light size-4.5" checked={table.getIsAllRowsSelected()} onChange={table.getToggleAllRowsSelectedHandler()} />,
      cell: ({ row }: { row: TableRow<SellerProductType> }) => <input type="checkbox" className="form-checkbox form-checkbox-light size-4.5" checked={row.getIsSelected()} onChange={row.getToggleSelectedHandler()} />,
      enableSorting: false,
    ...
    columnHelper.accessor('name', {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-base">
          <img src={row.original.image} alt="Product" className="size-9" />
          <h6>
  ...
  const [data, setData] = useState<SellerProductType[]>(() => [...sellerProductData])
