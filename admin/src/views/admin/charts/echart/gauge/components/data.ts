...
import { ECharts, EChartsOption, GaugeSeriesOption } from 'echarts'
import { CallbackDataParams } from 'echarts/types/dist/shared'

// basic gauge chart

export const getGaugeChart = (): EChartsOption => ({
  tooltip: {
    formatter: '{a} <br/>{b} : {c}%',
    padding: [7, 15],
...
export const getSpeedStageGaugeChart = (): EChartsOption => ({
  series: [
    {
      type: 'gauge',
      radius: '95%',
      axisLine: {
        lineStyle: {
          width: 30,
          color: [
            [0.3, getColor('chart-alpha')],
            [0.7, getColor('chart-beta')],
            [1, getColor('chart-alpha')],
          ],
        },
      },
...
export const getRingGaugeChart = (): EChartsOption => ({
  tooltip: {
    trigger: 'axis',
    padding: [7, 10],
    ...
    <div>
      <h6 class="fs-9 text-body-tertiary mb-0">
        <span class="fas fa-circle me-1" style='color:${item.color}'></span>
...
let temperatureInstance: ECharts | null = null

export function setTemperatureChartInstance(instance: ECharts): void {
  temperatureInstance = instance
}

let temperatureValue = +(Math.random() * 60).toFixed(2)

export const getTemperatureChart = (): EChartsOption => ({
  series: [
    {
      type: 'gauge',
      radius: '100%',
...
export function startTemperatureAnimation() {
  function run() {
    temperatureValue = +(Math.random() * 60).toFixed(2)
    if (temperatureInstance) {
      temperatureInstance.setOption({
        series: [{ data: [{ value: temperatureValue }] }, { data: [{ value: temperatureValue }] }],
      })
    }
  }
...
export const getMultiRingGaugeChart = (): EChartsOption => ({
  series: [
    {
      type: 'gauge',
      startAngle: 90,
...
export function setGaugeChartInstance(instance: ECharts): void {
  gaugeInstance = instance
}
interface GaugeDatum {
  value: number
  name: string
  title: { offsetCenter: [string, string] }
  detail: { offsetCenter: [string, string] }
}

export const gaugeData: GaugeDatum[] = [
  {
    value: 20,
    name: 'Good',
...
export const getMultiGaugeChart = (): EChartsOption => ({
  series: [
    {
      type: 'gauge',
      radius: '95%',
...
export function startGaugeAnimation() {
  function run() {
    for (let i = 0; i < gaugeData.length; i++) {
      gaugeData[i].value = +(Math.random() * 100).toFixed(2)
    }
    if (gaugeInstance) {
